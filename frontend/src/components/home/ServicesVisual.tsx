...
function MobileServicesVisual({ sectionRef }: { sectionRef: React.RefObject<HTMLElement | null> }) {
  useEffect(() => {
    const section = sectionRef.current;
    if (!section) return;

    const items = section.querySelectorAll<HTMLElement>('[data-mob-anim]');

    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            entry.target.classList.add('sv-mob--visible');
          }
        });
      },
  ...
  return (
    <section
      className="services-visual services-visual--mobile"
      ref={sectionRef as React.RefObject<HTMLElement>}
    >
      {/* Section Header */}
      <div className="sv-mob__header" data-mob-anim>
        <span className="sv-mob__eyebrow">Visual Service Discovery</span>
        <h2>Explore NAPCO's Print Capabilities</h2>
      </div>

      {/* Service Cards */}
      <div className="sv-mob__list">
        {visualServices.map((service) => (
          <article className="sv-mob__card" key={service.title} data-mob-anim>
            <div className="sv-mob__image-wrap">
              <img
                src={service.image}
                alt={service.title}
            ...
            <div className="sv-mob__copy">
              <h3 className="sv-mob__title">{service.title}</h3>
              <p className="sv-mob__desc">{service.description}</p>
            </div>
          </article>
        ))}
      </div>
      ...
      {/* CTA */}
      <div className="sv-mob__cta" data-mob-anim>
...
function DesktopServicesVisual({ sectionRef }: { sectionRef: React.RefObject<HTMLElement | null> }) {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      renderer.setSize(section.offsetWidth, section.offsetHeight, false);
    };

    const animate = () => {
      if (!isVisible) { frameId = 0; return; }
      material.uniforms.uTime.value += 0.016;
      renderer.render(scene, camera);
      frameId = window.requestAnimationFrame(animate);
    };

    const start = () => { if (!frameId) frameId = window.requestAnimationFrame(animate); };
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const introTitle  = section.querySelector<HTMLElement>('.services-visual__intro h2');
      const pill        = section.querySelector<HTMLElement>('.services-visual__intro span');
      const scrollHint  = section.querySelector<HTMLElement>('.services-visual__scroll-hint');

      const tiles       = Array.from(section.querySelectorAll<HTMLElement>('.services-visual__tile'));
      const gridLines   = Array.from(section.querySelectorAll<HTMLElement>('.services-visual__line'));
      const details     = Array.from(section.querySelectorAll<HTMLElement>('.services-visual__detail'));
      const detailImages= Array.from(section.querySelectorAll<HTMLElement>('.services-visual__detail-image'));
      const detailCopies= Array.from(section.querySelectorAll<HTMLElement>('.services-visual__detail-copy'));
      ...
      gsap.set(pill,        { autoAlpha: 1, y: 0, scale: 1 });
      gsap.set(introTitle,  { autoAlpha: 1, y: 24, scale: 0.96 });
      gsap.set(gridLines,   { autoAlpha: 0, scale: 0, transformOrigin: '50% 50%' });
      gsap.set(tiles,       { xPercent: -50, yPercent: -50, autoAlpha: 0, scale: 0.86, rotate: 0, transformOrigin: '50% 50%' });
      gsap.set(firstTile,   { autoAlpha: 0, scale: 1.58, y: 56, rotateX: 10, zIndex: 12 });
      gsap.set(otherTiles,  { autoAlpha: 0, scale: 0.82, y: 34 });
      gsap.set(details,     { autoAlpha: 0, pointerEvents: 'none' });
      gsap.set(detailImages,{ autoAlpha: 0, scale: 1.08, y: 64, rotateX: 7 });
      gsap.set(detailCopies,{ autoAlpha: 0, y: 36 });
      ...
      tl.addLabel('start');
      tl.to(gridLines,  { autoAlpha: 0.62, scale: 1, duration: 0.8, stagger: 0.05 }, 'start');
      tl.to(introTitle, { autoAlpha: 1, y: 0, scale: 1, duration: 0.55 }, 'start');
      tl.to(introTitle, { autoAlpha: 0, y: -90, scale: 0.96, duration: 0.75, ease: 'power3.inOut' }, 'start+=0.55');
      tl.to(pill,       { y: -130, scale: 0.94, duration: 0.75, ease: 'power3.inOut' }, 'start+=0.55');
      tl.to(firstTile,  { autoAlpha: 1, scale: 1.28, y: 0, rotateX: 0, duration: 1.05, ease: 'back.out(1.45)' }, 'start+=0.92');

      tl.addLabel('grid-build');
      tl.to(firstTile,  { scale: 1, y: 0, duration: 0.88, ease: 'power3.inOut' }, 'grid-build');
      tl.to(otherTiles, { autoAlpha: 1, scale: 1, y: 0, duration: 0.88, stagger: { amount: 0.36, from: 'center' }, ease: 'back.out(1.18)' }, 'grid-build+=0.16');
      tl.to(tiles,      {
        x: (i) => ([0, -10, 10, -14, 0, 14][i] || 0),
        y: (i) => ([0, -12, 10, 12, -10, 14][i] || 0),
        duration: 0.65, stagger: 0.03,
      }, 'grid-build+=0.82');

      tl.addLabel('grid-disperse');
      tl.to(pill,  { autoAlpha: 0, y: -24, duration: 0.4, ease: 'power2.out' }, 'grid-disperse');
      tl.to(tiles, {
        x: (i) => ([0, -560, 560, -620, 0, 620][i] || 0),
        y: (i) => ([0, -330, -310, 330, 380, 320][i] || 0),
        scale: 0.78,
        rotate: (i) => ([0, -6, 6, 8, -5, 7][i] || 0),
        autoAlpha: 0, duration: 1, stagger: { amount: 0.2, from: 'edges' }, ease: 'power3.inOut',
      }, 'grid-disperse+=0.05');
      tl.to(gridLines, { autoAlpha: 0.44, duration: 0.5 }, 'grid-disperse+=0.2');
      ...
      visualServices.forEach((_, index) => {
        ...
        tl.to(detail, { autoAlpha: 1, pointerEvents: 'auto', duration: 0.01 }, label);
        tl.to(image,  { autoAlpha: 1, scale: 1, y: 0, rotateX: 0, duration: 0.85, ease: 'power3.out' }, label);
        tl.to(copy,   { autoAlpha: 1, y: 0, duration: 0.72, ease: 'power3.out' }, `${label}+=0.12`);
        tl.to({}, { duration: 0.74 });

        if (index < visualServices.length - 1) {
          tl.to(image,  { autoAlpha: 0, scale: 0.94, y: -58, duration: 0.52, ease: 'power2.inOut' }, `${label}+=1.32`);
          tl.to(copy,   { autoAlpha: 0, y: -32, duration: 0.44, ease: 'power2.inOut' }, `${label}+=1.3`);
          tl.to(detail, { autoAlpha: 0, pointerEvents: 'none', duration: 0.01 }, `${label}+=1.86`);
        }
      });
    }, section);
...
export default function ServicesVisual() {
  ...
  // Swap the rendered sub-component based on breakpoint.
  // Each sub-component owns its own effects and cleans up fully on unmount,
  // so switching modes is completely clean — no stuck pins, no orphaned loops.
  return isMobile
    ? <MobileServicesVisual sectionRef={sectionRef} />
    : <DesktopServicesVisual sectionRef={sectionRef} />;
}
